	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (rho3d <= rho2d) {
				...
				atomicAdd(&dL_dmean2D_pose[global_id].x, dL_dG_pose * dG_ddelx); // not scaled
				atomicAdd(&dL_dmean2D_pose[global_id].y, dL_dG_pose * dG_ddely); // not scaled
	...
	glm::mat4* dL_dviews)
{
	...
	glm::mat3x4 dL_dM_aug_pose = P * glm::transpose(dL_dT_pose);

	glm::mat3 dL_dM_pose = glm::mat3(
		glm::vec3(dL_dM_aug_pose[0]),
		glm::vec3(dL_dM_aug_pose[1]),
		glm::vec3(dL_dM_aug_pose[2])
	);
#if DUAL_VISIABLE
	...
	glm::vec3 p_world = glm::vec3(p_origs[idx].x, p_origs[idx].y, p_origs[idx].z);
	glm::mat3 fake_RS = glm::mat3(L[0], L[1], p_world);
	glm::mat3 dL_dW_pose = dL_dM_pose * glm::transpose(fake_RS);
	glm::vec3 dL_dcam_pos_pose = dL_dM_pose[2];

	dL_dviews[idx][0] = glm::vec4(dL_dW_pose[0], 0.0);
	dL_dviews[idx][1] = glm::vec4(dL_dW_pose[1], 0.0);
	dL_dviews[idx][2] = glm::vec4(dL_dW_pose[2], 0.0);
	dL_dviews[idx][3] = glm::vec4(dL_dcam_pos_pose, 0.0);
