	...
	glm::mat4* dL_dprojs)
{
	...
	// T = glm::transpose(M) * world2ndc * ndc2pix;
	// glm: T = glm::transpose(M) * world_view_transform * projection_matrix * ndc2pix
	// real: T = ndc2pix.T @ projection_matrix.T @ world_view_transform.T @ M
	// real: dL_dview = (ndc2pix.T @ projection_matrix.T).T @ dL_dT @ M.T
	// real: dL_dview = projection_matrix @ ndc2pix @ dL_dT @ M.T
	...
	dL_dprojs[idx][2] = dL_dproj[2];
	dL_dprojs[idx][3] = dL_dproj[3];
